...
2. <a href="#create-a-batch-file-and-set-the-anaconda-prompt-to-always-run-as-an-administrator-windows-10">Create a Batch File to Launch Jupyter Lab Automatically</a>
3. <a href="#change-the-default-jupyter-notebook-default-browser">Change the Default Notebook Browser</a>
...
<li>FINISH THIS INSTRUCTION 14. In the prompt, type <code>git clone </code> and find your cloned repository
...
<li>Change the directory again using the command <code>cd GIS-3011-Setting-Up-Your-Environments</code></li>
<li>Create the needed environment by using the command <code>conda env create -f gis-3011.yml</code></li>
<ul><li>Solving and creating the environment may take up to 20 minutes, depending on your computer's hardware and space available</li></ul>
<li>Once the environment is created, you will need to activate said environment. You may choose to add to your Access Code .txt file the following command (using your path, obviously). This is necessary every time you start Jupyter Lab/Notebooks</li>
<ul><li>First, find your environment's path with the command <code>conda info --envs</code></li>
<li>Then determine if your environment was installed in one of the accessible conda environments with the command <code>conda config --show envs_dirs</code></li>
<li>If the allowed path and the installed path match, you're good to go. If it doesn't match, you'll need to add the path that *is* showing (everything before gis-3011) using this command <code>conda config --append envs_dirs /anaconda3/envs</code> (but do change it to the path you're showing). Then run the command <code>conda config --prepend envs_dirs /Users/<user>/anaconda3/envs</code> (be sure to change <user> to your user name!)</li>
...
<li>Then you can activate the environment with the command <code>activate gis-3011</code></li>
...
<li>When you use a Jupyter notebook in ArcGIS Pro and it's spinning and says "Initializing kernel", it is using the conda activate command</li></ul></ol>
 <strong>MAKE SURE YOU'VE ACTIVATED THE GIS-3011 ENVIRONMENT BEFORE CONTINUING!</strong>
 <ol start="17"><li>Install mamaba with the following command: <code>conda install mamba -c conda-forge</code></li>
...
<li>Alternatively, you can launch Anaconda Navigator instead of the Anaconda Prompt and select a Jupyter Notebook (do be sure you have the gis-3011 environment selected in the dropdown). Do be sure to launch Anaconda Navigator as an Administrator.</li>
<li>This will launch a Jupyter Notebook in Microsoft Edge, even if your default browser is Chrome or Firefox.</li></ul></ol>
<ol start="19"><li>Close out of Microsoft Edge and the Anaconda prompt.</li></ol>
</td></tr>
</tbody></table>

### Create a Batch File and Set the Anaconda Prompt to Always Run as an Administrator (Windows 10)
...
<tr><td><ol start="4"><li>RIGHT-CLICK the JupyterLaunch.bat file and select "Create Shortcut", renaming it something line JupyterLaunch.bat - Administrator</li></td><td>
...
### Change the Default Jupyter Notebook Default Browser
